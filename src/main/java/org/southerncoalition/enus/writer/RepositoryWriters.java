...
public class RepositoryWriters extends RepositoryWritersGen<Object> {   
	...
	protected void _repositorySimpleName(Wrap<String> c) {
		c.o(classSimpleName + "Repository");
	}

	protected void _repositoryImplSimpleName(Wrap<String> c) {
		c.o(classSimpleName + "RepositoryImpl");
	}

	protected void _pathDir(Wrap<String> c) {
		c.o(projectPathLinux + "/src/main/java/" + StringUtils.replace(repositoryPackageName, ".", "/"));
	}

	protected void _dir(Wrap<File> c) {
		File o = new File(pathDir);
		o.mkdirs();
		c.o(o);
	}

	protected void _pathRepository(Wrap<String> c) {
		c.o(pathDir + "/" + repositorySimpleName + ".java");
	}

	protected void _pathRepositoryImpl(Wrap<String> c) {
		c.o(pathDir + "/" + repositoryImplSimpleName + ".java");
	}
...
//		l("import com.citi.auth.model.AuthenticationMethod;");
			...
			l();
			lRepositoryImpl("@Repository");
			lRepositoryImpl("@Transactional");
			...
			if(classApiMethods != null) {
				for(String classApiMethod : classApiMethods) {
					...
					l();
					String classApiKeywordMethod = (String)solrDocumentClass.get("classApiKeyword" + classApiMethod + "_enUS_stored_string");
...
//					s("createNamedQuery");
					...
					if(StringUtils.contains(classApiMethod, "Search") || StringUtils.contains(classApiMethod, "GET") || StringUtils.contains(classApiMethod, "PUT") || StringUtils.contains(classApiMethod, "PATCH") || StringUtils.contains(classApiMethod, "DELETE")) {
						for(int i = 0; i < searchEntitiesResults.size(); i++) {
							...
							String entityVarSql = (String)entitySolrDocument.get("entityVarSql_stored_string");
							if(StringUtils.isBlank(entityVarSql))
								entityVarSql = StringUtils.upperCase(entityVar);
							sqlColumnName = sqlColumnName == null ? entityVar : sqlColumnName;

							if(entityKeywords == null)
								entityKeywords = new ArrayList<>();

							if(entityKeywords.contains(classApiMethod + ".request") && !entityKeywords.contains(classApiMethod + ".header")) {
								if(!javaParams.getEmpty()) {
									javaParams.s(", ");
									log1.s(", ");
								}
								javaParams.s(entitySimpleNameComplete, " ", entityVar);
								sqlParams.tl(4, ".setParameter( \"", entityVarSql, "\", ", entityVar, " )");
								log1.s(entityVar, ": {}");
								log2.s(", ", entityVar);
							}
